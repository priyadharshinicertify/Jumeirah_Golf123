//
//  FGalleryViewController1.m
//  Jumeirah_Golf_Estates
//
//  Created by Veeramani on 26/08/14.
...
@interface FGalleryViewController1 (Private)
...
@implementation FGalleryViewController1
...
- (void)removeImageAtIndex:(NSUInteger)index
{
	...
	FGalleryPhotoView *imgView = _photoViews[index];
 	FGalleryPhotoView *thumbView = _photoThumbnailViews[index];
...
- (void)layoutButtons
{
	...
	for (i = 0; i < count; i++) {
		UIBarButtonItem *btn = _barItems[i];
...
- (void)arrangeThumbs
{
	...
	[_thumbsView setContentSize:CGSizeMake( _thumbsView.frame.size.width - ( kThumbnailSpacing*4 ), dy + kThumbnailSize + kThumbnailSpacing )];
}
...
- (void)preloadThumbnailImages
{
	...
	photo = _photoLoaders[[NSString stringWithFormat:@"%lu", (unsigned long)index]];
	...
	{
		photo = _photoLoaders[[NSString stringWithFormat:@"%lu", (unsigned long)curIndex]];
...
- (FGalleryPhoto*)createGalleryPhotoForIndex:(NSUInteger)index
{
	FGalleryPhotoSourceType1
    sourceType = [_photoSource photoGallery:self sourceTypeForPhotoAtIndex:index];
	FGalleryPhoto *photo;
	NSString *thumbPath;
	NSString *fullsizePath;
	
	if( sourceType == FGalleryPhotoSourceTypeLocal1 )
	{
		thumbPath = [_photoSource photoGallery:self filePathForPhotoSize:FGalleryPhotoSizeThumbnail1 atIndex:index];
		fullsizePath = [_photoSource photoGallery:self filePathForPhotoSize:FGalleryPhotoSizeFullsize1 atIndex:index];
		photo = [[FGalleryPhoto alloc] initWithThumbnailPath:thumbPath fullsizePath:fullsizePath delegate:self];
	}
	else if( sourceType == FGalleryPhotoSourceTypeNetwork1 )
	{
		thumbPath = [_photoSource photoGallery:self urlForPhotoSize:FGalleryPhotoSizeThumbnail1 atIndex:index];
		fullsizePath = [_photoSource photoGallery:self urlForPhotoSize:FGalleryPhotoSizeFullsize1 atIndex:index];
...
- (void)galleryPhoto:(FGalleryPhoto*)photo willLoadThumbnailFromPath:(NSString*)path
{
	...
	if( _isThumbViewShowing ) {
		FGalleryPhotoView *thumb = _photoThumbnailViews[photo.tag];
...
- (void)galleryPhoto:(FGalleryPhoto*)photo didLoadThumbnail:(UIImage*)image
{
	...
	FGalleryPhotoView *thumbView = _photoThumbnailViews[photo.tag];
...
{
	if([self.visibleViewController isKindOfClass:[FGalleryViewController1 class]])
	{
        return YES;
	}
...
- (void)willRotateToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration
{
	...
	if([self.visibleViewController isKindOfClass:[FGalleryViewController1 class]])
	{
		FGalleryViewController1 *galleryController = (FGalleryViewController1*)self.visibleViewController;
...
- (BOOL)shouldAutorotateToInterfaceOrientation:(UIInterfaceOrientation)interfaceOrientation
{
    ...
    if( [self.selectedViewController isKindOfClass:[UINavigationController class]])
    {
        ...
        // see if the current controller in the stack is a gallery
        if([navController.visibleViewController isKindOfClass:[FGalleryViewController1 class]])
        {
            return YES;
        }
    }
...
- (void)willRotateToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration
{
	if([self.selectedViewController isKindOfClass:[UINavigationController class]])
	{
		...
		if([navController.visibleViewController isKindOfClass:[FGalleryViewController1 class]])
		{
			FGalleryViewController1 *galleryController = (FGalleryViewController1*)navController.visibleViewController;
			[galleryController resetImageViewZoomLevels];
		}
	}
}
