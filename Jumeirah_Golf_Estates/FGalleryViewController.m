...
- (void)loadView
{
    ...
    if([manage.colorchnage isEqualToString:@"hello"])
    {
        self.title = @"Back";
        ...
        UIColor *barColour = [UIColor whiteColor];
        [[UINavigationBar appearance] setBarTintColor:barColour];
        
        manage.colorchnage=@"";
        
    }
    if([manage.golf_1 isEqualToString:@"h2"])
    {
        self.title = @"Earth";
        self.navigationController.navigationBar.barStyle = UIBarStyleDefault;
...
- (void)viewWillDisappear:(BOOL)animated
{
	...
	//[[UIApplication sharedApplication] setStatusBarStyle:nil animated:animated];
}
...
- (void)unloadFullsizeImageWithIndex:(NSUInteger)index
{
	if (index < [_photoViews count]) {		
		FGalleryPhoto *loader = _photoLoaders[[NSString stringWithFormat:@"%lu", (unsigned long)
                                                             index]];
