    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if ( imageExists ) {    
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                imageView.image = [UIImage imageWithContentsOfFile:[cache[AsynchronousFreeloaderCachePaths] valueForKey:link]];
            });
...
+ (void)removeImageFromCache:(NSString *)link
{
   ...
   dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
       ...
       NSMutableDictionary *pathsDictionary = [NSMutableDictionary dictionaryWithDictionary:cache[link]];
       [pathsDictionary removeObjectForKey:link];
       
       // Save name to namesArray of cache (used to retain position)
       NSMutableArray *namesArray = [NSMutableArray arrayWithArray:cache[link]];
       [namesArray removeObject:link];
       
       // Save pathsDictionary and namesArray to cache
       cache[AsynchronousFreeloaderCachePaths] = pathsDictionary;
       cache[AsynchronousFreeloaderCacheNames] = namesArray;
...
+ (NSMutableDictionary*)createReferenceToCache
{
    ...
    if ( [[NSUserDefaults standardUserDefaults] objectForKey:AsynchronousFreeloaderCache] ) {
        ...
        cache[AsynchronousFreeloaderCachePaths] = [NSMutableDictionary dictionary];
        cache[AsynchronousFreeloaderCacheNames] = [NSMutableArray array];
        [[NSUserDefaults standardUserDefaults] setObject:cache forKey:AsynchronousFreeloaderCache];
        [[NSUserDefaults standardUserDefaults] synchronize];
    }
...
+ (BOOL)doesImageWithName:(NSString *)name existInCache:(NSMutableDictionary *)cache
{
    ...
    BOOL imageExists = NO;
    NSMutableArray *namesArray = [NSMutableArray arrayWithArray:cache[AsynchronousFreeloaderCacheNames]];
    NSMutableDictionary *pathsDictionary = [NSMutableDictionary dictionaryWithDictionary:cache[AsynchronousFreeloaderCachePaths]];
...
+ (void)performGarbageCollection
{
    ...
    if  ( [namesArray count] >= AsynchronousFreeloaderCacheSize ) {
        ...
        while (limitExcessionAmount > 0) {
            ...
            [pathsDictionary removeObjectForKey:namesArray[0]];
